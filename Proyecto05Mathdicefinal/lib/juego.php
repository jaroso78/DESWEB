...
class dado3{
    ...
    function getdado(){
        echo ("<img width='100px' heigth='100px' src='".$this-> array[$this-> tirada]."' class='dado3'value='".$this-> tirada."'> </>");
...
class juegoSimple{
    ...
    function __construct($jugada,$valor){
    $this->jugada = $jugada;
    $this->valor = $valor;
    }
    
    function resultado($puntuacion){
        
        if($this->jugada == $this->valor){
            echo "<input type='button' value='CORRECTO' class='btn btn-success  btn-block' id='btnHome' onClick= document.location.href='./pantalla_juego.php' /> ";
            $puntuacion++;
            
           
        }else
        {
            echo "<input type='button' value='INCORRECTO' class='btn btn-warning  btn-block' id='btnHome' onClick= document.location.href='./pantalla_juego.php' /> ";
        }
        return $puntuacion;
    }
}
